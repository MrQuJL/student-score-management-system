...
public class TeacherServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		TeacherService tecService = new TeacherService();
		User user = (User) request.getSession().getAttribute("user");
		String account = user.getAccount();

		if ("LoginOut".equals(method)) { // �˳�ϵͳ
			loginOut(request, response);
		} else if ("toTeacherPersonalView".equals(method)) { // ����ʦ������Ϣ����

			int id = tecService.getTeacherId(account);// ��ȡ��ʦid
			List<Teacher24> list = tecService.getTeacher(id);// ��ȡobj����
			String realPath = request.getSession().getServletContext()
					.getRealPath("");
			tecService.getBlob(id, realPath);

			for (int i = 0; i < list.size(); i++) {
				request.setAttribute("tec" + i, list.get(i));// ����ݵ�ǰ̨
			}
			boolean flag=false;
			request.setAttribute("flag", flag);
			request.setAttribute("photoname", "1");
			request.getRequestDispatcher(
					"/WEB-INF/view/teacher/teacherPersonal.jsp").forward(
					request, response);
			...
			String msg = null;
			try {
				tecService.update(sex, phone, qq, number);
				msg = "success";
			} catch (Exception e) {
				msg = "false";
				e.printStackTrace();
			...
			String psd = tecService.getPsd(account);// ԭʼ����
			...
			if (psd.equals(oldPsd)) {
				if (newPsd.equals(rePsd)) {
					try {
						tecService.updatePsd(account, newPsd);
						msg = "success";
					} catch (Exception e) {
						msg = "false";
						e.printStackTrace();
			...
			try {
				...
				for (FileItem item : list) {
					if (item.isFormField()) {
						...
						String newName = new Date().getTime() + "_" + rad + "."
								+ type;
    					photoname = newName;
				...
				/*
				 * 1. ��һ�� ��ѯ��������� �ŵ�TeacherBean2. request.setAt.....( "teacher"
				 * , bean ); request.setAt.....( "picPath" , ������µ�·�� );3.
				 * JSPҳ��Ϳ�����EL���ʽ ��ֵ
				 */
				int id = tecService.getTeacherId(account);// ��ȡ��ʦid
				List<Teacher24> list1 = tecService.getTeacher(id);// ��ȡobj����
				String realPath1 = request.getSession().getServletContext()
						.getRealPath("");
				tecService.getBlob(id, realPath1);

				for (int i = 0; i < list1.size(); i++) {
					request.setAttribute("tec" + i, list1.get(i));// ����ݵ�ǰ̨
				}
				boolean flag=true;
				request.setAttribute("flag", flag);
                request.setAttribute("photoname", photoname);
                
                tecService.insertphoto(id, path.replaceAll("\\\\", "/")); 
                request.getRequestDispatcher(
						"/WEB-INF/view/teacher/teacherPersonal.jsp").forward(
						request, response);
